...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagPower_on)
    {
        info->event[info->num] = EVENT_Power_on;
        info->num++;
        dataPoints->valuePower_on = gizStandardDecompressionValue(Power_on_BYTEOFFSET,Power_on_BITOFFSET,Power_on_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagPower_off)
    {
        info->event[info->num] = EVENT_Power_off;
        info->num++;
        dataPoints->valuePower_off = gizStandardDecompressionValue(Power_off_BYTEOFFSET,Power_off_BITOFFSET,Power_off_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagReset)
    {
        info->event[info->num] = EVENT_Reset;
        info->num++;
        dataPoints->valueReset = gizStandardDecompressionValue(Reset_BYTEOFFSET,Reset_BITOFFSET,Reset_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuePower_on != cur->valuePower_on)
    {
        GIZWITS_LOG("valuePower_on Changed\n");
        ret = 1;
    }
    if(last->valuePower_off != cur->valuePower_off)
    {
        GIZWITS_LOG("valuePower_off Changed\n");
        ret = 1;
    }
    if(last->valueReset != cur->valueReset)
    {
        GIZWITS_LOG("valueReset Changed\n");
        ret = 1;
    }

    if(last->valueCPU_DATA != cur->valueCPU_DATA)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueCPU_DATA Changed\n");
            ret = 1;
        }
    }
    if(last->valueMemory_DATA != cur->valueMemory_DATA)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueMemory_DATA Changed\n");
            ret = 1;
        }
    }
    if(last->valueCPU2_DATA != cur->valueCPU2_DATA)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueCPU2_DATA Changed\n");
            ret = 1;
        }
    }
    if(last->valueCPU3_DATA != cur->valueCPU3_DATA)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueCPU3_DATA Changed\n");
            ret = 1;
        }
    }
    if(last->valueCPU4_DATA != cur->valueCPU4_DATA)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueCPU4_DATA Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(Power_on_BYTEOFFSET,Power_on_BITOFFSET,Power_on_LEN,(uint8_t *)devStatusPtr,dataPoints->valuePower_on);
    gizStandardCompressValue(Power_off_BYTEOFFSET,Power_off_BITOFFSET,Power_off_LEN,(uint8_t *)devStatusPtr,dataPoints->valuePower_off);
    gizStandardCompressValue(Reset_BYTEOFFSET,Reset_BITOFFSET,Reset_LEN,(uint8_t *)devStatusPtr,dataPoints->valueReset);
    gizByteOrderExchange((uint8_t *)devStatusPtr->wBitBuf,sizeof(devStatusPtr->wBitBuf));

    devStatusPtr->valueCPU_DATA = gizY2X(CPU_DATA_RATIO,  CPU_DATA_ADDITION, dataPoints->valueCPU_DATA); 
    devStatusPtr->valueMemory_DATA = gizY2XFloat(Memory_DATA_RATIO,  Memory_DATA_ADDITION, dataPoints->valueMemory_DATA); 
    devStatusPtr->valueCPU2_DATA = gizY2X(CPU2_DATA_RATIO,  CPU2_DATA_ADDITION, dataPoints->valueCPU2_DATA); 
    devStatusPtr->valueCPU3_DATA = gizY2X(CPU3_DATA_RATIO,  CPU3_DATA_ADDITION, dataPoints->valueCPU3_DATA); 
    devStatusPtr->valueCPU4_DATA = gizY2X(CPU4_DATA_RATIO,  CPU4_DATA_ADDITION, dataPoints->valueCPU4_DATA); 
